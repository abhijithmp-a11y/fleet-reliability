          ...
          <tbody className="divide-y divide-slate-200">
            {table.paginatedData.map((cluster, i) => (
              <tr key={i} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className="flex items-center gap-1 bg-slate-100 px-1.5 py-0.5 rounded text-[10px] font-medium text-slate-700 w-fit">
                    <Cpu size={10} className="text-slate-400" /> {cluster.gpuType}
                  </span>
                </td>
                ...
                <td className="px-4 py-3">
                  <button 
                    ...
                    className={`flex items-center gap-1.5 font-bold hover:underline transition-all ${
                      cluster.compliance === 'green' ? 'text-emerald-600' : 
                      cluster.compliance === 'yellow' ? 'text-amber-600' : 'text-rose-600'
                    }`}
                  >
                    ...
                    {cluster.compliance === 'green' ? 'Passed' : cluster.compliance === 'yellow' ? 'Warning' : 'Failed'}
